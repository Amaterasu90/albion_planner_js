class TableColumnFactory {
    createColumns = (items, propertyDefinitions) => {
        ...
        return propertyDefinitions.map((definition, index) => { return { dataField: definition.id, text: definition.name, headerStyle: { width: definition.width }, hidden: definition.hidden, key: `${definition.id}_${definition.name}_${index}` } });
    }
}
