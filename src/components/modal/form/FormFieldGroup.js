...
class FormFieldGroup extends React.Component {
    ...
    getInputs = (props, addRow, countEmelentsInLine, index, index_1) => {
        ...
        if (props.type === "range") {
            ...
            input = (<RelatedAsyncSelect key={`${props.id}_${index}_${index_1}_related`} xs={size} xxl={size} {...props} />)
        } else if (props.type === "relatedHidden") {
            input = (<RelatedAsyncSelect key={`${props.id}_${index}_${index_1}_related`} xs={size} xxl={size} {...props} />)
        } else if (props.type === "asyncRelatedDropdownLabeled") {
            input = (<Col xs={size} xxl={size}>
                <FormLabel>{this.props.placeholder}</FormLabel>
                <RelatedAsyncSelect key={`${props.id}_${index}_${index_1}_related`} xs={size} xxl={size} {...props} />
            </Col>)
            ...
            input = (<Col xs={size} xxl={size}><FormControl key={`${props.id}_${index}_${index_1}_form`} {...props} /></Col>)
        }
